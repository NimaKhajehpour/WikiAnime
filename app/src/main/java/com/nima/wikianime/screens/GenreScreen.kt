...
import com.nima.wikianime.viewmodel.GenreViewModel
...
fun GenreScreen (
    ...
    var allGenreMedias: ApolloResponse<AllGenreMediasQuery.Data>? by remember {
        mutableStateOf(null)
    }

    LaunchedEffect(key1 = page){
        launch{
            allGenreMedias = viewModel.getAllGenreMedias(page, genre!!)
        }
    }
    ...
    Column(
        ...
        Surface(
            ...
            Column(
                ...
                AnimatedVisibility(visible = allGenreMedias != null) {
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                        ...
                        Text(text = "$page/${allGenreMedias?.data?.Page?.pageInfo?.lastPage} Pages",
                        ...
                        IconButton(onClick = {
                            ...
                            enabled = page != allGenreMedias?.data?.Page?.pageInfo?.lastPage &&
                                    allGenreMedias?.data?.Page?.pageInfo?.hasNextPage == true
                        ) {
                        ...
                        Spacer(modifier = Modifier.weight(1f))
                        IconButton(onClick = {
                            page = allGenreMedias?.data?.Page?.pageInfo?.lastPage!!
                        },
                            enabled = page != allGenreMedias?.data?.Page?.pageInfo?.lastPage &&
                                    allGenreMedias?.data?.Page?.pageInfo?.hasNextPage == true
                        ) {
        ...
        AnimatedVisibility(visible = allGenreMedias != null) {
            if (allGenreMedias != null){
                LazyColumn(
                    ...
                    items(items = allGenreMedias?.data?.Page?.media!!) {
                        ThumbnailItemDetailed(media = it!!,
                            onClick = { id ->
                                      navController.navigate(Screens.MediaScreen.name+"/$id")
                            },
                            onGenreClicked = {genreString ->
                               if (genreString != genre) { navController.navigate(Screens.GenreScreen.name + "/$genreString") }
                            }
                        )
                    }
                }
