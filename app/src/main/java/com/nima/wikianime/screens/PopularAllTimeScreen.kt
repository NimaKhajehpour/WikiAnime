...
import com.nima.wikianime.viewmodel.PopularAllTimeViewModel
import kotlinx.coroutines.launch

@Composable
fun PopularAllTimeScreen (
    navController: NavController,
    viewModel: PopularAllTimeViewModel
){
...
//    }.value
    ...
    var popularAllTime: ApolloResponse<PopularAllTimeQuery.Data>? by remember {
        mutableStateOf(null)
    }

    LaunchedEffect(key1 = page){
        launch{
            popularAllTime = viewModel.getPopularAllTime(page)
        }
    }
            ...
            Column(
                ...
                AnimatedVisibility(visible = popularAllTime != null) {
                    Row(
                        ...
                        Text(text = "$page/${popularAllTime?.data?.Page?.pageInfo?.lastPage} Pages",
                        ...
                        IconButton(onClick = {
                            ...
                            enabled = page != popularAllTime?.data?.Page?.pageInfo?.lastPage &&
                                    popularAllTime?.data?.Page?.pageInfo?.hasNextPage == true
                        ) {
                        ...
                        Spacer(modifier = Modifier.weight(1f))
                        IconButton(onClick = {
                            page = popularAllTime?.data?.Page?.pageInfo?.lastPage!!
                        },
                            enabled = page != popularAllTime?.data?.Page?.pageInfo?.lastPage &&
                                    popularAllTime?.data?.Page?.pageInfo?.hasNextPage == true
                        ) {
